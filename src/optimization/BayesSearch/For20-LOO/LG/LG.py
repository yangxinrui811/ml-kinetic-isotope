...
optimal_params = OrderedDict([
    ...
    ('verbose', -1)  # 不打印每次迭代的详细信息
])

model = LGBMRegressor(**optimal_params)
...
# 将所有结果保存到CSV文件
results_df.to_csv("G:/BaiduSyncdisk/0.我的工作/5.隧穿相图/1.正式代码/2.代码/BayesSearch/For20-LOO/LG/results.csv", index=False)
average_results_df.to_csv("G:/BaiduSyncdisk/0.我的工作/5.隧穿相图/1.正式代码/2.代码/BayesSearch/For20-LOO/LG/average_results.csv", index=False)