...
optimal_params = {
    ...
    'n_estimators': 100,
    'subsample': 1.0,
    'random_state': 42,  # 添加随机种子以确保结果的可重复性
    'verbosity': 0  # 不打印每次迭代的详细信息
}
...
for i in range(k_folds):
    ...
    # 如果目标变量是对数尺度的，使用下面的偏差计算；否则，根据需要调整
...
# 将所有结果保存到CSV文件
results_df.to_csv("G:/BaiduSyncdisk/0.我的工作/5.隧穿相图/1.正式代码/2.代码/BayesSearch/For20-LOO/XB/results.csv", index=False)
average_results_df.to_csv("G:/BaiduSyncdisk/0.我的工作/5.隧穿相图/1.正式代码/2.代码/BayesSearch/For20-LOO/XB/average_results.csv", index=False)