...
# 数据集定义
df = pd.read_csv("D:/BaiduSyncdisk/0.坚果云/2.隧穿相图/测试/data.csv", sep=',')  # 假设CSV文件使用逗号分隔
...
# 定义贝叶斯优化搜索空间
param_space = {
    'C': Real(1e-3, 1e3, 'log-uniform'),  # 正则化参数
    'gamma': Real(1e-3, 1e3, 'log-uniform'),  # 核函数系数
    'epsilon': Real(1e-4, 1e1, 'log-uniform')  # ε-不敏感损失函数中的ε参数
}
