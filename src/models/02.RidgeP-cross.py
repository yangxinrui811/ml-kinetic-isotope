...
# 使用岭回归模型，并通过交叉验证找到最佳参数  
param_grid = {  
    'alpha': [0.1, 1.0, 10.0, 100.0]  # 正则化强度的取值范围  
}  
  
grid_search = GridSearchCV(Ridge(solver='saga'), param_grid, cv=10, scoring='neg_mean_squared_error')  
...
# 绘制测试集的散点图
plt.scatter(y_test, y_test_pred, color='white', marker='o', edgecolor='blue', alpha=1, label='Test Set')
